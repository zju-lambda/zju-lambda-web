      ...
      if (data.repository && data.repository.name &&
        ...
        cp.exec(cmd, function(err) {
          ...
          var postData = {
            'msgtype': 'text',
            'text': {
              'content': data.repository.name + ' ' + committer + ': ' + msg
            }
          };
          ...
          data.repository && data.repository.name &&
          data.repository.name == 'dualdb') {
        if (s.length >= 2 && s[0] == 'branch') {
          branch = ['cd dualdb', 'git checkout ' + s[1], 'cd ../'];
        }
        var committer = 'who';
        if (data && data.head_commit && data.head_commit.committer) {
          committer = data.head_commit.committer.name;
        ...
        var cmds =
            [
              'rm -rf engine dualdb',
              'git clone git@git.dev.tencent.com:Nicekingwei/dualdb.git'
            ].concat(branch)
                .concat([
                  'git clone git@code.aliyun.com:nicekingwei/engine.git',
                  'rm -rf engine/engine_race/*.h',
                  'rm -rf engine/engine_race/*.cc',
                  'cp -r dualdb/engine_race/include/* engine/engine_race',
                  'cp -r dualdb/engine_race/src/* engine/engine_race',
                  'cd engine', 'cp -r include/ engine_race/', 'git add .',
                  'git commit -m "' + msg + '"', 'git push', 'cd ../',
                  'rm -rf engine dualdb'
                ]);
        ...
        cp.exec(cmd, function(err) {
          ...
          data.repository && data.repository.name &&
          data.repository.name == 'codb') {
        if (s.length >= 2 && s[0] == 'branch') {
          branch = ['cd codb', 'git checkout ' + s[1], 'cd ../'];
        }
        var committer = 'who';
        if (data && data.head_commit && data.head_commit.committer) {
          committer = data.head_commit.committer.name;
        ...
        var cmds =
            [
              'rm -rf engine codb',
              'git clone git@git.dev.tencent.com:Nicekingwei/codb.git'
            ].concat(branch)
                .concat([
                  'git clone git@code.aliyun.com:nicekingwei/engine.git',
                  'rm -rf engine/engine_race/*.h',
                  'rm -rf engine/engine_race/*.cc',
                  'cp -r codb/engine_race/include/* engine/engine_race',
                  'cp -r codb/engine_race/src/* engine/engine_race',
                  'cd engine', 'cp -r include/ engine_race/', 'git add .',
                  'git commit -m "' + msg + '"', 'git push', 'cd ../',
                  'rm -rf engine codb'
                ]);
